function Get-SDPProblemTask {
    <#
    .SYNOPSIS
        Retrieves tasks for a ServiceDesk Plus problem.
    .PARAMETER ProblemId
    ...
    .EXAMPLE
        Get-SDPProblemTask -ProblemId '12345' -Id '1'
    #>
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'Id') {
            $response = Invoke-SDPRestMethod -Endpoint "problems/$ProblemId/tasks/$Id"
        ...
        if ($All) {
            ...
            do {
                ...
                $response = Invoke-SDPRestMethod -Endpoint "problems/$ProblemId/tasks" -InputData @{ list_info = $listInfo }
                foreach ($t in $response.tasks) { [SDPProblemTask]::new($ProblemId, $t) }
                $index += $PageSize
