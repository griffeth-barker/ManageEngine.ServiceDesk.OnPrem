function Get-SDPProblemNote {
    <#
    .SYNOPSIS
        Retrieves notes for a ServiceDesk Plus problem.
    .PARAMETER ProblemId
    ...
    .EXAMPLE
        Get-SDPProblemNote -ProblemId '12345' -Id '67890'
    #>
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'Id') {
            $response = Invoke-SDPRestMethod -Endpoint "problems/$ProblemId/notes/$Id"
        ...
        if ($All) {
            ...
            do {
                ...
                $response = Invoke-SDPRestMethod -Endpoint "problems/$ProblemId/notes" -InputData @{ list_info = $listInfo }
                foreach ($n in $response.notes) { [SDPProblemNote]::new($ProblemId, $n) }
                $index += $PageSize
