function Get-SDPChangeTask {
    <#
    .SYNOPSIS
        Retrieves tasks for a ServiceDesk Plus change.
    .PARAMETER ChangeId
    ...
    .EXAMPLE
        Get-SDPChangeTask -ChangeId '12345' -Id '1'
    #>
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'Id') {
            $response = Invoke-SDPRestMethod -Endpoint "changes/$ChangeId/tasks/$Id"
        ...
        if ($All) {
            ...
            do {
                ...
                $response = Invoke-SDPRestMethod -Endpoint "changes/$ChangeId/tasks" -InputData @{ list_info = $listInfo }
                foreach ($t in $response.tasks) { [SDPChangeTask]::new($ChangeId, $t) }
                $index += $PageSize
