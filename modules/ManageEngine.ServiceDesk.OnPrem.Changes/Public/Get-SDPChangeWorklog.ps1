function Get-SDPChangeWorklog {
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'Id') {
            $response = Invoke-SDPRestMethod -Endpoint "changes/$ChangeId/worklogs/$Id"
        ...
        if ($All) {
            ...
            do {
                ...
                $response = Invoke-SDPRestMethod -Endpoint "changes/$ChangeId/worklogs" -InputData @{ list_info = $listInfo }
                foreach ($w in $response.worklogs) { [SDPChangeWorklog]::new($ChangeId, $w) }
                $index += $PageSize
