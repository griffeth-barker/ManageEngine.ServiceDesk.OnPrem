function Get-SDPMode {
    <#
    .SYNOPSIS
        Retrieves one or more modes from ServiceDesk Plus.
    .PARAMETER Id
        The ID of the mode to retrieve.
    .PARAMETER PageSize
    ...
    process {
        if ($PSCmdlet.ParameterSetName -eq 'Id') {
            $response = Invoke-SDPRestMethod -Endpoint "modes/$Id"
        ...
        if ($All) {
            ...
            do {
                ...
                $response = Invoke-SDPRestMethod -Endpoint 'modes' -InputData @{ list_info = $listInfo }
                foreach ($r in $response.modes) { [SDPReference]::new($r) }
                $index += $PageSize
